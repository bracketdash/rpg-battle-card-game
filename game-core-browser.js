...
(function () {
  ...
  function applyDamage(target, dmg, attacker) {
    ...
    if (tgt.dodge) {
      tgt.dodge = false;
      return { target: tgt, attacker: atk, result: "dodged" };
    }
    if (tgt.counter && atk) {
      tgt.counter = false;
      atk.hp -= dmg;
  ...
  function stealCard(fromHand, toHand) {
    const from = fromHand.slice();
    const to = toHand.slice();
    if (from.length === 0) return { from, to, stolen: null };
    const stolen = from.pop();
    to.push(stolen);
  ...
  function absorbTransfer(actor, donors, requests, cap) {
    ...
    requests.forEach((r) => {
      donorsCopy[r.donorIndex].hp = Math.min(
        donorsCopy[r.donorIndex].maxHP,
        donorsCopy[r.donorIndex].hp + r.amt,
      );
    });
    actorCopy.hp -= total;
    if (actorCopy.hp < 1) {
