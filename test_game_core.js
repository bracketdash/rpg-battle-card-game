...
function testAttackAll(){
  ...
  const targets = [{name:'a', hp:5, maxHP:5},{name:'b', hp:1, maxHP:5}];
...
function testAbsorbEdge(){
  ...
  const donors = [{hp:3,maxHP:10,isKO:false},{hp:6,maxHP:8,isKO:false}];
  // donors damage: 7 and 2 => avail donor[0]=7, donor[1]=2
  const req = [{donorIndex:0, amt:2},{donorIndex:1, amt:2}];
