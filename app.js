...
(function(){
  ...
  const CARD_TEMPLATES = [
    {name:'Attack', type:'attack', mult:1, count:13, desc:'Deal STR damage to one character.'},
    {name:'Attack II', type:'attack', mult:2, count:10, desc:'Deal STR x2 to one character.'},
    {name:'Attack III', type:'attack', mult:3, count:8, desc:'Deal STR x3 to one character.'},
    {name:'Attack All', type:'attackAll', mult:1, count:9, desc:'Deal STR to all opposing characters.'},
    {name:'Attack All II', type:'attackAll', mult:2, count:3, desc:'Deal STR x2 to all opposing characters.'},
    {name:'Heal', type:'heal', mult:1, count:5, desc:'Heal MAG HP from one character.'},
    {name:'Heal II', type:'heal', mult:2, count:3, desc:'Heal MAG x2.'},
    {name:'Heal III', type:'heal', mult:3, count:2, desc:'Heal MAG x3.'},
    {name:'Heal All', type:'healAll', mult:1, count:2, desc:'Heal MAG from all friendly characters.'},
    {name:'Heal All II', type:'healAll', mult:2, count:1, desc:'Heal MAG x2 from all friendly characters.'},
    {name:'Absorb', type:'absorb', mult:1, count:4, desc:'Move up to MAG damage from elsewhere to self.'},
    {name:'Absorb II', type:'absorb', mult:2, count:3, desc:'Move up to MAG x2.'},
    {name:'Absorb III', type:'absorb', mult:3, count:2, desc:'Move up to MAG x3.'},
    {name:'Revive', type:'revive', mult:1, count:7, desc:'Revive and heal up to MAG HP.'},
    {name:'Revive II', type:'revive', mult:2, count:3, desc:'Revive and heal up to MAG x2.'},
    {name:'Discard', type:'discard', mult:1, count:4, desc:'Opponent discards 1 card.'},
    {name:'Discard II', type:'discard', mult:2, count:2, desc:'Opponent discards 2 cards.'},
    {name:'Draw', type:'draw', mult:1, count:4, desc:'You may draw an additional card.'},
    {name:'Draw II', type:'draw', mult:2, count:2, desc:'Draw up to 2 additional cards.'},
    {name:'Steal', type:'steal', mult:1, count:3, desc:'Take a card from opponent hand.'},
    {name:'Dodge', type:'dodge', mult:1, count:7, desc:'Use when attacked to take zero damage.'},
    {name:'Counter', type:'counter', mult:1, count:3, desc:'Use when attacked to reverse the attack.'}
  ];
  ...
  function createEmptyPlayer(num){
    return {id:num, chars:[], hand:[], usedThisTurn:[], name:`Player ${num}`};
  }
  ...
  function renderDraft(){
    ...
    state.draftPool.forEach((c,idx)=>{
      ...
      if(picksLeft > 0){ el.classList.add('clickable'); el.addEventListener('click',()=> pickChar(idx)); }
  ...
  const drafted1 = $('#drafted-1'); const drafted2 = $('#drafted-2');
  drafted1.innerHTML = ''; drafted2.innerHTML = '';
  state.players[0].chars.forEach(c=>{ const el = document.createElement('div'); el.className='char-card'; el.innerHTML = `<div class="name">${c.name}</div><div class="small-meta">HP ${c.maxHP}</div>`; drafted1.appendChild(el); });
  state.players[1].chars.forEach(c=>{ const el = document.createElement('div'); el.className='char-card'; el.innerHTML = `<div class="name">${c.name}</div><div class="small-meta">HP ${c.maxHP}</div>`; drafted2.appendChild(el); });
    ...
    if(totalPicked === state.draftPicksPerPlayer*2){ startGameBtn.disabled = false; startGameBtn.style.display = ''; }
    else { startGameBtn.disabled = true; startGameBtn.style.display = 'none'; }
  }

  function pickChar(idx){
    const char = state.draftPool.splice(idx,1)[0];
    // clone
    const charInstance = Object.assign({}, char, {maxHP:char.hp, hp:char.hp, isKO:false});
  ...
  function startGame(){
    ...
    state.players.forEach(p=>{
      ...
      p.chars.forEach(c=>{ c.dodge = false; c.counter = false; });
    });
    // deal 5 each
    for(let i=0;i<5;i++){
    ...
    state.currentPlayer = (typeof state.draftTurn === 'number') ? (1 - state.draftTurn) : 0;
  ...
  function updateUI(){
    ...
    updateStatus(`${state.players[state.currentPlayer].name}'s turn — Deck: ${state.deck.length} Discard: ${state.discard.length}`);
  ...
  function evaluateSkipVisibility(){
    ...
    let idx = state.nextCharIndex; while(idx < p.chars.length && (p.chars[idx].isKO || p.usedThisTurn.includes(idx))) idx++;
    const actor = p.chars[idx];
    if(!actor){ skipActionBtn.style.display = 'none'; return; }
    // if any card in hand is playable for this actor, hide skip
    const playable = p.hand.some(card => isCardPlayable(card, {player:state.currentPlayer, actorIdx: idx}));
    if(playable) skipActionBtn.style.display = 'none'; else skipActionBtn.style.display = '';
  }
  ...
  function isCardPlayable(card, ctx){
    ...
    switch(card.type){
      ...
      case 'absorb':
        ...
        return state.players.some((pl,side)=> pl.chars.some((ch,ci)=>{
          if(side===ctx.player && ci===ctx.actorIdx) return false; return (!ch.isKO && (ch.maxHP - ch.hp)>0);
        }));
      default:
        return false;
    }
  ...
  function setActivePlayerVisual(idx){
    ...
    playerPanels.forEach((el,i)=>{ el.classList.remove('active'); el.classList.remove('pulse'); });
  ...
  function renderPlayers(){
    for(let i=0;i<2;i++){
      ...
      const el = playerCharsEl[i]; el.innerHTML='';
      p.chars.forEach((c,idx)=>{
        ...
        if(i === state.currentPlayer){
          let nextIdx = state.nextCharIndex; while(nextIdx < p.chars.length && (p.chars[nextIdx].isKO || p.usedThisTurn.includes(nextIdx))) nextIdx++;
        ...
        ce.innerHTML = `<div class="name">${c.name}${c.isKO?'<span class="ko-badge">KO</span>':''}${usedBadge}</div>
  ...
  function renderHand(){
    ...
    player.hand.forEach((c,idx)=>{
      const ce = document.createElement('div'); ce.className='card small';
      ce.dataset.idx = idx;
  ...
  function renderNextChar(){
    ...
    if(idx >= p.chars.length){ nextCharEl.textContent = 'None (all used)'; state.nextCharResolved = true; }
    else { nextCharEl.textContent = `${p.chars[idx].name}`; state.nextCharResolved = false; }
  }
  ...
  function selectCard(idx){
    ...
    renderHand();
    updateStatus('Select a target if necessary, or press End Action to skip using the card.');
  ...
  function highlightTargets(card){
    ...
    if(card.type==='attack' || card.type==='attackAll'){
      ...
      if(card.type==='attackAll') updateStatus('Attack All will hit all opposing characters. Click End Action to confirm.');
      else{
        ...
        targetPanel.querySelectorAll('.char-card').forEach((el,i)=>{
          ...
          if(!opp.chars[i].isKO) el.onclick = ()=> selectTarget({side:1-state.currentPlayer, idx:i});
        });
      ...
      targetPanel.querySelectorAll('.char-card').forEach((el,i)=>{
        el.classList.toggle('selected', !current.chars[i].isKO || card.type==='revive');
        el.onclick = ()=> selectTarget({side:state.currentPlayer, idx:i});
      });
      updateStatus('Click a friendly character to target (or End Action to skip).');
    } else {
      updateStatus('Card selected — press End Action to play it (or Pass).');
    }
  }

  function selectTarget(t){ state.selectedTarget = t; updateStatus(`Target selected: ${state.players[t.side].chars[t.idx].name}`); }

  // Action resolution
  function resolveSelectedCard(){
    const p = state.players[state.currentPlayer];
    const cardIdx = state.selectedCardIdx;
    if(cardIdx === null){ updateStatus('No card selected. Choose a card to play or press Skip Action to pass.'); return; }
    // ensure there is an available actor
    const actor = getCurrentActor();
    if(!actor || actor.name === '(no actor)'){
      updateStatus('No available actor to perform actions; end your turn or pass.');
      return;
    }
    ...
    switch(card.type){
      ...
      case 'dodge':
        if(!state.selectedTarget){ updateStatus('Select a friendly character to place Dodge on.'); return; }
        const dTarget = state.players[state.selectedTarget.side].chars[state.selectedTarget.idx];
        dTarget.dodge = true;
        log(`${p.name} placed DODGE on ${dTarget.name}. It will cancel the next incoming attack.`);
        break;
      case 'counter':
        if(!state.selectedTarget){ updateStatus('Select a friendly character to place Counter on.'); return; }
        const cTarget = state.players[state.selectedTarget.side].chars[state.selectedTarget.idx];
        cTarget.counter = true;
        log(`${p.name} placed COUNTER on ${cTarget.name}. It will reflect the next incoming attack back.`);
        break;
      case 'attack':
        if(!state.selectedTarget){ updateStatus('Select a target to attack.'); return; }
        ...
        if(res.result === 'dodged') log(`${attacker.name} uses ${card.name} — ${target.name} dodged the attack!`);
        else if(res.result === 'countered'){
          // apply attacker changes
          attacker.hp = res.attacker.hp; attacker.isKO = !!res.attacker.isKO;
          log(`${attacker.name} uses ${card.name} — ${target.name} countered! ${attacker.name} takes ${attacker.maxHP - attacker.hp} damage.`);
        } else {
          const dmg = (attacker.str||0) * (card.mult||1);
          log(`${attacker.name} uses ${card.name} — ${dmg} damage to ${target.name} (HP ${Math.max(0,target.hp)}/${target.maxHP})`);
        }
        break;
      case 'attackAll':
        const opp = state.players[1-state.currentPlayer];
        ...
        results.forEach((r,i)=>{
          const ch = opp.chars[i]; ch.hp = r.target.hp; ch.isKO = !!r.target.isKO;
          if(r.result === 'dodged') log(`${actor.name} uses ${card.name} — ${ch.name} dodged.`);
          else if(r.result === 'countered'){ actor.hp = r.attacker.hp; actor.isKO = !!r.attacker.isKO; log(`${ch.name} countered — ${actor.name} took damage.`); }
          else log(`${actor.name} uses ${card.name} — ${actor.str*(card.mult||1)} to ${ch.name} (HP ${ch.hp}/${ch.maxHP})`);
        });
        break;
      case 'heal':
        if(!state.selectedTarget){ updateStatus('Select a friendly target to heal.'); return; }
        ...
        log(`${healer.name} uses ${card.name} — healed ${targetH.name} ${resH.healed || 0} (HP ${targetH.hp}/${targetH.maxHP})`);
        break;
      case 'healAll':
        ...
        healResults.forEach((r,i)=>{ if(!team.chars[i].isKO){ team.chars[i].hp = r.target.hp; log(`${healerA.name} heals ${team.chars[i].name} for ${r.healed} (HP ${team.chars[i].hp}/${team.chars[i].maxHP})`); } });
        break;
      case 'revive':
        if(!state.selectedTarget){ updateStatus('Select a friendly target to revive.'); return; }
        const revT = state.players[state.selectedTarget.side].chars[state.selectedTarget.idx];
        if(!revT.isKO){ updateStatus('That character is already active.'); return; }
        ...
        break;
      case 'draw':
        const num = card.mult||1; drawCardTo(p, num); log(`${p.name} drew ${num} card(s).`); break;
      case 'discard':
        ...
        log(`${p.name} forced ${oppP.name} to discard ${dres.removed.length} card(s).`);
        break;
      case 'steal':
  ...
  function applyDamage(target, dmg, reason){
    ...
    if(target.counter && arguments.length >= 4){
      ...
      log(`${reason} — ${target.name} countered! Reflecting ${dmg} to ${attacker.name}.`);
      // apply damage to attacker
      attacker.hp -= dmg;
      if(attacker.hp < 1){ attacker.isKO = true; attacker.hp = 0; log(`${attacker.name} is KO'd by counter!`); }
      target.counter = false;
      return;
    }

    target.hp -= dmg;
    log(`${reason} — ${dmg} damage to ${target.name} (HP ${Math.max(0,target.hp)}/${target.maxHP})`);
    if(target.hp < 1){ target.isKO = true; target.hp = 0; log(`${target.name} is KO'd!`); }
  }

  function applyHeal(target, amt, reason){
    if(target.isKO){ log(`Cannot heal ${target.name}; they are KO'd.`); return; }
    const prev = target.hp;
    target.hp = Math.min(target.maxHP, target.hp + amt);
    log(`${reason} — healed ${target.name} ${target.hp - prev} (HP ${target.hp}/${target.maxHP})`);
  }
  ...
  function markCharUsed(){
    ...
    let idx = state.nextCharIndex;
    while(idx < p.chars.length && (p.chars[idx].isKO || p.usedThisTurn.includes(idx))) idx++;
    if(idx < p.chars.length){ p.usedThisTurn.push(idx); state.nextCharIndex = idx+1; }
    updateUI();
    ...
    if(p.usedThisTurn.length >= activeCount){ log(`${p.name} has used all active characters.`); }
    // Also update status panel and auto-end turn when all characters used
    if(p.usedThisTurn.length >= activeCount){ updateStatus(`${p.name} — all active characters used. Ending turn...`); saveState(); setTimeout(()=> endTurn(), 600); }
    else { saveState(); }
  }
  ...
  window.addEventListener('keydown',(e)=>{
    if(e.key==='/') { log('---'); }
    if(e.key==='d') { // draw 1 for current player
      drawCardTo(state.players[state.currentPlayer],1); updateUI(); log('Drew 1 card (shortcut).');
    }
     // Undo / Redo keyboard shortcuts (Ctrl/Cmd+Z, Ctrl/Cmd+Y)
     if((e.ctrlKey || e.metaKey) && e.key && e.key.toLowerCase() === 'z'){
       e.preventDefault(); if(typeof undo === 'function') undo();
     }
     if((e.ctrlKey || e.metaKey) && e.key && e.key.toLowerCase() === 'y'){
       e.preventDefault(); if(typeof redo === 'function') redo();
     }
  });
  ...
  if(!restored){ startDraft(); saveState(); }
  ...
  let modalListeners = [];
  function addModalListener(el, evt, fn){ el.addEventListener(evt, fn); modalListeners.push({el,evt,fn}); }
  function clearModalListeners(){ modalListeners.forEach(l=>{ try{ l.el.removeEventListener(l.evt, l.fn); }catch(e){} }); modalListeners = []; }
  function closeModal(){ modal.classList.add('hidden'); try{ modalBody.innerHTML = ''; }catch(e){} // defer removal so any currently-dispatched handlers still run
  ...
  function persistHistory(){
    ...
    }catch(e){ console.warn('persistHistory failed', e); }
  }
  ...
  function loadPersistedHistory(){
    try{
      ...
      updateUndoRedoButtons();
      return true;
    }catch(e){ console.warn('loadPersistedHistory failed', e); return false; }
  }

  function applySnapshot(snap, opts={recordHistory:false}){
    // apply a snapshot (same shape as getSnapshot)
    state.players = snap.players.map(p=>({ id:p.id, name:p.name, chars:p.chars.map(c=>({ name:c.name, hp:c.hp, maxHP:c.maxHP, str:c.str, mag:c.mag, isKO:!!c.isKO, dodge:!!c.dodge, counter:!!c.counter })), hand:p.hand.slice(), usedThisTurn:p.usedThisTurn.slice() }));
  ...
  function getSnapshot(){
    ...
    return {
      players: state.players.map(p=>({ id:p.id, name:p.name, chars:p.chars.map(c=>({ name:c.name, hp:c.hp, maxHP:c.maxHP, str:c.str, mag:c.mag, isKO:!!c.isKO, dodge:!!c.dodge, counter:!!c.counter })), hand:p.hand.slice(), usedThisTurn:p.usedThisTurn.slice() })),
  ...
  function saveState(recordHistory=true){
    try{
      ...
      try{ const picks = (snap.players[0].chars.length||0) + (snap.players[1].chars.length||0); log(`Saved snapshot — picks ${picks}`); }catch(e){}
      updateUndoRedoButtons();
  ...
  function loadState(){
    try{
      ...
      try{ const picks = (snap.players[0].chars.length||0) + (snap.players[1].chars.length||0); log(`Loaded snapshot from storage — picks ${picks}`); }catch(e){}
      ...
      const totalPicked = state.players[0].chars.length + state.players[1].chars.length;
      if(totalPicked >= state.draftPicksPerPlayer*2){ draftScreen.classList.add('hidden'); gameScreen.classList.remove('hidden'); }
      else { draftScreen.classList.remove('hidden'); gameScreen.classList.add('hidden'); }
      updateUI(); renderDraft();
      ...
      try{
        if(history && history.length){
          ...
          if(picksInSaved >= snap.draftPicksPerPlayer * 2 && picksInLast < snap.draftPicksPerPlayer * 2){
            ...
            log('Restored draft-in-progress from persisted history (prefer draft over auto-start).');
            return true;
          }
        }
      }catch(e){ console.warn('history selection failed', e); }
    updateUndoRedoButtons();
  ...
  // Undo / Redo actions
  function undo(){
    if(history.length <= 1){ updateStatus('Nothing to undo.'); return; }
  ...
  function redo(){
    if(redoStack.length === 0){ updateStatus('Nothing to redo.'); return; }
  ...
  if(typeof undoBtn !== 'undefined' && undoBtn) undoBtn.addEventListener('click', undo);
  if(typeof redoBtn !== 'undefined' && redoBtn) redoBtn.addEventListener('click', redo);

  function openAbsorbModal(card, player, cardIdx){
    // actor is current actor
    const actorIdx = (()=>{ let i=state.nextCharIndex; while(i<player.chars.length && (player.chars[i].isKO || player.usedThisTurn.includes(i))) i++; return i; })();
    const actor = player.chars[actorIdx];
    if(!actor){ updateStatus('No available actor to perform Absorb.'); return; }
    ...
    if(donors.length===0){ updateStatus('No valid donors with damage to absorb from.'); return; }

    modalTitle.textContent = `Absorb — ${actor.name} can absorb up to ${cap} damage`;
    modalBody.innerHTML = '';
    const info = document.createElement('div'); info.className='small-meta'; info.textContent = 'Enter amounts to move from each donor (total must be ≤ cap).';
    ...
    donors.forEach(d=>{
      ...
      const input = document.createElement('input'); input.type='number'; input.min=0; input.max=d.damage; input.value=0;
      input.dataset.side = d.side; input.dataset.idx = d.idx;
      row.appendChild(name);
      row.appendChild(input);
      modalBody.appendChild(row);
    });
    // live total
    const totalEl = document.createElement('div'); totalEl.className='small-meta'; totalEl.style.marginTop='8px'; totalEl.textContent = 'Total: 0';
    modalBody.appendChild(totalEl);

    function updateTotal(){
      const inputs = Array.from(modalBody.querySelectorAll('input'));
      const total = inputs.reduce((s,i)=>s + Math.max(0, Number(i.value)||0),0);
      totalEl.textContent = `Total: ${total} (cap ${cap})`;
      if(total>cap) totalEl.style.color='var(--lose)'; else totalEl.style.color='var(--muted)';
    }
  ...
  function confirm(){
      ...
      if(total>cap){ alert('Total exceeds absorb cap. Reduce amounts.'); return; }
      // validate donors still have that much damage
      for(const r of requests){ const ch = state.players[r.side].chars[r.idx]; const avail = ch.maxHP - ch.hp; if(r.amt>avail){ alert('A donor no longer has that much damage available.'); return; } }
      // use core absorb to compute new actor and donors
      const donorsList = [];
      state.players.forEach((pl,side)=> pl.chars.forEach(ch=> donorsList.push({hp:ch.hp, maxHP:ch.maxHP, isKO:!!ch.isKO})));
      ...
      const flatRequests = requests.map(r=>{
        // compute flat index
        let flatIndex = 0;
        for(let s=0;s<r.side;s++) flatIndex += state.players[s].chars.length;
        flatIndex += r.idx;
      ...
      const actorFlatIndex = (()=>{ let i=0; let count=0; while(i<state.players.length){ if(i===state.currentPlayer){ // actor is at player.current's chars[actorIdx]
          break; } i++; } return 0; })();
      ...
      const actorIdxLocal = (()=>{ let i=state.nextCharIndex; const p = player; while(i<p.chars.length && (p.chars[i].isKO || p.usedThisTurn.includes(i))) i++; return i; })();
      const actorForCore = { hp: actor.hp, maxHP: actor.maxHP };
      const coreRequests = flatRequests.map(fr=>({ donorIndex: fr.donorIndex, amt: fr.amt }));
      let coreRes;
      try{ coreRes = window.gameCore.absorbTransfer(actorForCore, flatDonors, coreRequests, cap); }
      catch(e){ alert('Absorb failed: '+e.message); return; }
