...
function testSimple(){
  ...
  const requests = [{donorIndex:0, amt:3}, {donorIndex:1, amt:2}];
...
function testInvalid(){
  ...
  let threw = false;
  try{ absorbTransfer(actor, donors, [{donorIndex:0, amt:1}], 5); } catch(e){ threw = true; }
  assert(threw, 'Should throw when donor is KO');
  console.log('testInvalid passed');
}

testSimple(); testKOActor(); testInvalid();
